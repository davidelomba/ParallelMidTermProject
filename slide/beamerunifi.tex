...
% --- SLIDE 6 ---
\begin{frame}{Benchmarking Methodology}
  \begin{itemize}
    \item \textbf{Target Hardware:} Rigorous execution on a physical Quad-Core CPU to limit fictitious hyper-threading influence
    \item \textbf{Dataset:} 200 heterogeneous images extracted from BSDS500
    \item \textbf{Automated Pre-processing:} Python pipeline for uniform conversion and upscaling
    \item \textbf{Evaluation Metrics:}
      \begin{itemize}
        \item \textit{Strong Scaling:} Maintaining workload (2x/4x) while scaling physical thread count
        \item \textit{Weak Scaling:} Balanced and proportional increase of computing resources and resolution (data volume)
      \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Strong \& Weak Scalability Analysis}
  \begin{columns}
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
          \footnotesize
          \item Constant execution times when doubling workload and cores
          \item Confirms excellent hardware resource distribution
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}
    \begin{column}{0.45\textwidth}
      \begin{itemize}
        ...
        \item \textbf{Peak Efficiency:} yields up to a \textbf{1.67x} performance boost over the standard parallel baseline
...
\begin{frame}{Peak Performance \& Future Developments}
  \begin{columns}
    ...
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        \item \textbf{Hardware/Software Conclusion:} Bare multithreading cannot save a flawed algorithm. Hardware/software integration is the only path to High-Performance
        \item \textbf{Heterogeneous Computing (Future Work):}
          \begin{itemize}
            \item Porting the logic to GPU architectures (CUDA)
            \item The SIMT model and hardware Shared Memory would resolve vertical memory cache asymmetries
          \end{itemize}
      \end{itemize}
    \end{column}
  \end{columns}
