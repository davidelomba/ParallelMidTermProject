...
Weak scaling was measured by increasing both the image resolution (from \textbf{1.0x} to \textbf{4.0x}) and the thread count proportionally. Ideally, the execution time should remain constant, yielding an efficiency of 1.0. However, the experimental data shows a gradual increase in execution time, with the parallel efficiency dropping from 1.0 down to approximately \textbf{0.4} at 8 threads. This behavior confirms that as the data volume grows, the spatial locality of the cache hierarchy degrades; the overhead of managing a lot of threads in a Fork-Join model and the strict RAM bandwidth limits for large matrices, makes perfect weak scaling unachievable for this specific memory-bound workload.
...
Dilation and Erosion: across all kernel sizes and resolutions, Dilation 
systematically achieves approximately twice the speedup of Erosion. Inspecting 
...
ranging from approximately \textbf{1.2x} to \textbf{1.7x} depending on the image 
...
threads the gain peaks at roughly \textbf{1.55x} (scale 1.0x) and \textbf{1.67x} 
...
\textbf{1.67x} for compound operations at low thread counts, while explicit 
